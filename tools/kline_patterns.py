...
class KLinePatternRecognizer:
    ...
    def _recognize_hammer(self, df: pd.DataFrame) -> Optional[PatternResult]:
        ...
        # 锤子线条件
        if (
            info["lower_pct"] >= 0.5  # 下影线长
            and info["body_pct"] <= 0.3  # 实体小
            and info["upper_pct"] <= 0.1
        ):  # 上影线短
            # 检查是否在下跌趋势（前5天总体下跌）
    ...
    def _recognize_inverted_hammer(self, df: pd.DataFrame) -> Optional[PatternResult]:
        ...
        if (
            info["upper_pct"] >= 0.5
            and info["body_pct"] <= 0.3
            and info["lower_pct"] <= 0.1
        ):
            trend = self._get_trend(df, 5)
            if trend == "down":
                return PatternResult(
    ...
    def _recognize_engulfing_bull(self, df: pd.DataFrame) -> Optional[PatternResult]:
        ...
        # 前阴后阳，且后包住前
        if (
            prev["close"] < prev["open"]  # 前一日阴线
            and curr["close"] > curr["open"]  # 当日阳线
            and curr_body_high >= prev_body_high  # 上包
    ...
    def _recognize_engulfing_bear(self, df: pd.DataFrame) -> Optional[PatternResult]:
        ...
        if (
            prev["close"] > prev["open"]  # 前一日阳线
            and curr["close"] < curr["open"]  # 当日阴线
    ...
    def _recognize_spinning_top(self, df: pd.DataFrame) -> Optional[PatternResult]:
        ...
        if (
            info["body_pct"] <= 0.2
            and info["upper_pct"] >= 0.2
            and info["lower_pct"] >= 0.2
        ):
            return PatternResult(
                code="spinning_top",
                name="纺锤线",
    ...
    def _recognize_three_white_soldiers(
        ...
        if (
            ...
            if d3["close"] > d2["close"] > d1["close"]:
                ...
                if (
                    d1["open"] < d2["open"] < d1["close"]
                    and d2["open"] < d3["open"] < d2["close"]
                ):
                    return PatternResult(
