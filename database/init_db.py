...
def init_database():
    ...
    try:
        with conn.cursor() as cursor:
            # 创建数据库
            cursor.execute("CREATE DATABASE IF NOT EXISTS stock_review CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
            print("[OK] Database stock_review created")
            
            # 使用数据库
            cursor.execute("USE stock_review")
            ...
            for i, sql in enumerate(tables_sql, 1):
                try:
                    cursor.execute(sql)
                    print(f"[OK] Table {i} created")
                    success_count += 1
                except Exception as e:
                    if '1050' in str(e) or 'already exists' in str(e):
                        print(f"[SKIP] Table {i} already exists")
                        success_count += 1
                    else:
                        print(f"[ERR] Table {i}: {e}")
            
            print(f"\n{'='*50}")
            print(f"Tables: {success_count}/{len(tables_sql)} ready")
            ...
            for code, name, ptype, desc, ver in patterns:
                try:
                    cursor.execute("""
                        INSERT INTO kline_pattern_dict (pattern_code, pattern_name, pattern_type, description, version)
                        VALUES (%s, %s, %s, %s, %s)
                        ON DUPLICATE KEY UPDATE pattern_name=VALUES(pattern_name)
                    """, (code, name, ptype, desc, ver))
                except Exception as e:
                    print(f"[WARN] Pattern {code}: {e}")
            
            print("[OK] K-line patterns inserted")
            
            # 验证表
            cursor.execute("SHOW TABLES")
            tables = cursor.fetchall()
            print(f"\nTables in stock_review:")
            for t in tables:
